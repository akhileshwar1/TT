...
exports.RoadSnapper = class {
  constructor(
    ...
    rawLocationsRef.on('value', snapshot => {
    ...
    if (valSnapshot && this.time) {
      ...
      tripnames.forEach(tripname => {
          ...
          lng= this.snapped.hasOwnProperty(tripname) &&
            ...
            const request = {origin: [lat,lng], destination: 'Five gardens,Matunga'};
          var response = _await(this.googleMapsClient.directions(request).asPromise())
      .json;
